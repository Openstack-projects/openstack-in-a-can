...
{{- define "helpers.template.overlay" -}}
  ...
  {{- if empty (index $.Global.Values ( printf "_helper_template_overlay_%s" $.template_definition )) -}}
    {{- $local := dict -}}
    {{/*
    ...
    */}}
    ...
    {{- if hasKey $.Global.Values.overrides ( index $overlay_keys 0 ) -}}
      {{- if hasKey ( index $.Global.Values.overrides ( index $overlay_keys 0 ) ) ( index $overlay_keys 1 ) -}}
        {{- $_ := set $local "overlay" ( index $.Global.Values.overrides ( index $overlay_keys 0 ) ( index $overlay_keys 1 ) )  -}}
      {{ end }}
    {{ end }}
    ...
    {{- $_ := set $.Global.Values ( printf "_helper_template_overlay_%s" $.template_definition ) $target -}}
  {{ end }}

  {{/*
  we use json as the output here, as the toYaml function is non-deterministic
  */}}
  {{ index $.Global.Values ( printf "_helper_template_overlay_%s" $.template_definition ) | toPrettyJson }}
{{- end -}}


{{- define "helpers.template.merge" -}}
  {{- $local := dict -}}
